...
const TimeRangeItem = forwardRef(({range}: {range: TimeRange}, ref: React.Ref<ItemHandle>) => {
...
const SectionDepartureTimes = forwardRef<SectionDepartureTimesHandle>((_, ref) => {
    ...
    useImperativeHandle(ref, () => ({
        getState: () => TIME_RANGES.filter((range) => refs.current[range.label]?.current?.get()),
        setState: (selectedRanges) => {
            TIME_RANGES.forEach((range) =>
                refs.current[range.label]?.current?.set(selectedRanges.some((r) => r.label === range.label))
            );
        },
        reset: () => {
            TIME_RANGES.forEach((range) => refs.current[range.label]?.current?.set(false));
        },
    }));
